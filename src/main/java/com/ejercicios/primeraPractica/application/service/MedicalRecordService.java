...
public class MedicalRecordService {
	...
	public Optional<String> addMedicalRecord(MedicalRecordRequest medicalRecordRequest) {
		...
		// Validamos que la persona existe
		if (personOpt.isPresent()) {
			Person person = personOpt.get();

			// Validar que el nutricionista existe
			Optional<Person> nutritionistOpt = personRepository
					.findByPersoInfoDocument(medicalRecordRequest.getNutritionistId());
			if (nutritionistOpt.isPresent() && nutritionistOpt.get().getType() == PersonType.NUTRITIONIST) {
				...
				exitId = Optional.ofNullable(savedMedicalRecord.getId());

				// Actualizamos la persona con el id generado de la ficha
				Person personForSave = personOpt.get();
